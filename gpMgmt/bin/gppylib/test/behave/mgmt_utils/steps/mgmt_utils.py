...
def populate_regular_table_data(context, tabletype, table_name, compression_type, dbname, rowcount=1094, with_data=False, host=None, port=0, user=None):
    ...
    if compression_type == "None":
        create_partition(context, table_name, tabletype, dbname, compression_type=None, partition=True,
                         rowcount=rowcount, with_data=with_data, host=host, port=port, user=user)
    else:
        create_partition(context, table_name, tabletype, dbname, compression_type, partition=True,
                         rowcount=rowcount, with_data=with_data, host=host, port=port, user=user)
...
def impl(context, tabletype, table_name, compression_type, dbname, with_data, HOST, PORT, USER):
    ...
    populate_regular_table_data(context, tabletype, table_name, compression_type, dbname, 10, with_data, host=host, port=port, user=user)
