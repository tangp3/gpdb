...
def drop_external_table(context, table_name, dbname, host=None, port=0, user=None):
    ...
    with dbconn.connect(dbconn.DbURL(hostname=host, port=port, usrname=user, dbname=dbname)) as conn:
        dbconn.execSQL(conn, SQL)
        conn.commit()
...
def create_partition(context, tablename, storage_type, dbname, compression_type=None, partition=True, rowcount=10, with_data=False, host=None, port=0, user=None):
    ...
    with open('/tmp/nihao', 'w') as fw:
        fw.write('%s' % with_data)
...
def populate_partition(tablename, start_date, dbname, data_offset, rowcount=1094, host=None, port=0, user=None):
    ...
    with open('/tmp/wo', 'w') as fw:
        fw.write(insert_sql_str)
