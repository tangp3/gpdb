...
""")
    ...
    def test_getdumptables_from_remote_host(self, m1, m2, mockCommand_init):
            ...
            result = get.get_dump_tables()
            mockCommand_init.assert_called_once_with('Get remote copy of dumped tables', 'ssh -o \'StrictHostKeyChecking no\' remote_host  cat myfile   | grep -e "-- Name: " -e "^\\W*START (" -e "^\\W*PARTITION " -e "^\\W*DEFAULT PARTITION " -e "^\\W*SUBPARTITION " -e "^\\W*DEFAULT SUBPARTITION "')
    ...
    def test_getdumptables_from_remote_host_compressed_file(self, m1, m2, mockCommand_init):
            ...
            result = get.get_dump_tables()
            mockCommand_init.assert_called_once_with('Get remote copy of dumped tables', 'ssh -o \'StrictHostKeyChecking no\' remote_host  cat myfile.gz  | gunzip  | grep -e "-- Name: " -e "^\\W*START (" -e "^\\W*PARTITION " -e "^\\W*DEFAULT PARTITION " -e "^\\W*SUBPARTITION " -e "^\\W*DEFAULT SUBPARTITION "')
