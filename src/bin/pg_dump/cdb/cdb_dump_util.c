            ...
            bool createStorageUnit, char **default_backup_directory, bool remote)
{
	...
	char *log_level = NULL;
	char *log_size = NULL;
	char *storage_unit_configed = NULL;
	...
	err = getDDBoostCredential(&dd_boost_hostname,
			...
			&storage_unit_configed,
			remote);


	if (*ddboost_storage_unit == NULL)
		*ddboost_storage_unit = Safe_strdup(storage_unit_configed);

	if(storage_unit_configed)
		free(storage_unit_configed);
